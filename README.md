...
扫描目录后，界面会加载图像缩略图网格，并在顶部显示文件总数。点击任意卡片可在右侧打开单条编辑面板，进行 caption 查看、修改与保存。

| 区域 | 说明 |
|------|------|
| 文件统计 | 扫描完成后显示 **共 N 个文件**，便于确认数据集规模 |
| 缩略图网格 | 每张卡片展示图像预览，并标注对应的 `.png` / `.txt` 文件名 |
| 单条编辑面板 | 右侧显示选中图像的大图预览、文本格式（TXT 等）及 caption 编辑区 |
| 单条翻译 | 编辑面板内可单独翻译当前文本，不影响其他条目 |
| 保存 / 关闭 | 修改后点击 **保存** 写回文件，或 **关闭** 返回网格视图 |

![扫描目录后的素材浏览与单条编辑](docs/images/editor-with-data.png)
